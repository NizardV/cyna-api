...
* **Namespace `Webzine.Repository`** dans `EfSlowQueryInterceptor` à corriger (voir section 5) — actuellement la traçabilité "quel repository a déclenché cette requête lente" est inopérante.
* Aucune transaction explicite n'enveloppe les écritures multi-tables critiques (ex. création de commande, voir `05-Panier-Commandes.md`) — `SaveChangesAsync()` est appelé séquentiellement par étape.
* Le fichier `Application/Services/temp.cs` est vide et présent dans le repository — probable fichier de travail oublié, à supprimer pour garder l'arborescence propre.
